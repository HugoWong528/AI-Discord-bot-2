...
- [Pricing](#pricing)
- [Part 1 — Deploy the General Chat Bot](#part-1--deploy-the-general-chat-bot)
...
- [Part 2 — Deploy the AI Company Bot](#part-2--deploy-the-ai-company-bot)
- [Deploying Both Bots in One Project](#deploying-both-bots-in-one-project)
...
## Part 1 — Deploy the General Chat Bot
...
2. Go to **Settings → Source → Root Directory** and set it to:
   ...
   general-chat
   ```
   This tells Railway to treat `general-chat/` as the project root.  Railway's nixpacks builder will then find `requirements.txt` and `runtime.txt` automatically.

3. Verify the **Start Command** field is either empty (Railway reads it from `railway.toml`) or manually set to:
   ```
   python bot.py
   ```
   The included `general-chat/railway.toml` already specifies this command, so no manual entry is required.
...
## Part 2 — Deploy the AI Company Bot

The AI Company bot is deployed exactly the same way as the General Chat bot, but using a **different root directory and different secrets**.

1. In your Railway project, click **New Service → GitHub Repo** and pick your forked repo again (or click **+ New** inside the existing project to add a second service).
...
4. Add environment variables:
   ...
   | `GITHUB_TOKEN` | GitHub personal access token with `repo` write scope | `/build` command only |
...
## Deploying Both Bots in One Project

Railway supports multiple services inside a single project.  Running both bots in one project lets you share variables and view both log streams in the same dashboard.
...
| `POLLINATIONS_TOKEN` | ✅ | Pollinations AI API key |
| `GITHUB_TOKEN` | ⚠️ `/build` only | GitHub token with `repo` write scope |
...
Railway uses nixpacks to detect and install Python dependencies.  Make sure `requirements.txt` is present in the service's root directory (`general-chat/requirements.txt` or `ai-company/requirements.txt`).

If you see this error, verify the **Root Directory** is set correctly:

```
Service settings → Source → Root Directory → general-chat
```
